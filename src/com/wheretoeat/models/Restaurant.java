...
public class Restaurant {
	...
	public static Restaurant fromJson(JSONObject jsonObject){
		...
		try {
			rest.phoneNumber = jsonObject.getString("phone");
			rest.rating = jsonObject.getDouble("rating");
			rest.imageUrl = jsonObject.getString("image_url");
			rest.name = jsonObject.getString("name");
			rest.categories = Category.parseJSONCategoryList(jsonObject.getJSONArray("categories"));
			rest.ratingImageUrl = jsonObject.getString("rating_img_url");
			rest.reviewCount = jsonObject.getInt("review_count");
			rest.location = Location.fromJson(jsonObject.getJSONObject("location"));
			rest.displayPhoneNumber = jsonObject.getString("display_phone");
		} catch (JSONException e) {
			e.printStackTrace();
			return null;
		}
